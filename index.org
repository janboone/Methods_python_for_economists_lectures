...
<th style="padding:8px;border:1px solid #ddd;">Economic Topic</th>
<th style="padding:8px;border:1px solid #ddd;">Core Economic Idea</th>
<th style="padding:8px;border:1px solid #ddd;">Python / Computational Skills</th>
</tr>
</thead>
<tbody>
...
*** Question 4
**** Economic intuition: Compounding and Labor Market Flows
...
Suppose we have an inelastic labor supply normalized to a mass of 1. Labor can be in one of three states: unemployed, employed in a low wage job and employed in a high wage job.
...
*** Question 1
**** Adverse Selection: Does Increasing Mark-Up Always Restore Trade?
...
*** Question 2
**** Cournot Duopoly: Add a Third Firm
...
*** Question 3
**** Labor Market: Steady State Without Iteration
...
*** Question 4
**** Matrix Notation: Simulation With Numpy Only
...
*** Question 5
**** Extending the Labor Model: Add a Direct High Wage Offer Channel
...
The goal is not to obtain a precise prediction. Rather, we want to give the minister a sense of the *order of magnitude* of the effects. To build intuition, we construct an interactive model where immigration can increase by anywhere between 0% and 100%.
...
*** 8. Interactive immigration shock (marimo slider)

In the app, immigration is controlled with a slider.  
In =marimo= we create the same interaction using a widget.
...
Many economic problems involve decisions and adjustments that unfold over time. Investment today affects future production, firms learn from past experience, and strategic decisions often depend on expectations about future outcomes. To analyze such situations economists use dynamic models.

Dynamic models are frequently expressed using differential equations that describe how economic variables evolve as a function of their current values. Although these models can sometimes be solved analytically, numerical methods are often needed to compute their behavior and visualize their implications.
...
#+begin_src python
alpha = mo.ui.slider(0.1,0.9,value=0.3)
...
even if the equation does not depend on =t=.
...
- how economists represent dynamic processes using differential equations  
- how to simulate dynamic systems numerically using Python  
...
The app above allows you to explore these dynamics interactively.
...
The phase diagram shows the loci where \( \dot{k}=0 \) and \( \dot{c}=0 \), as well as the trajectory implied by the initial conditions. The app also computes the *saddle path*, which is the unique trajectory that leads to the steady state.
...
- =numpy= provides numerical tools and array operations.
- =matplotlib= is used to create graphs.
- =solve_ivp= from =scipy.integrate= numerically solves systems of differential equations.
- =marimo= provides interactive widgets such as sliders.
...
*** Question 1
**** Ramsey model: steady state and saddle path
...
*** Question 2
**** Ramsey dynamics and adjustment paths
...
*** Question 3
**** Predator–prey model: steady state and cycles
...
*** Question 4
**** Economic interpretation of predator–prey dynamics
...
*** Question 5
**** From predator–prey to a second‑order differential equation
...
*** Question 6
**** Conceptual question: Stability and economic dynamics
